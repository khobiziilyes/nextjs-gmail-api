...
export default function Home({
  ...
  const handleAuthSuccess = (uuid: string) => {
    setUuid(uuid);
  };
  const handleOpenApi = () => {
    window.open(`/api/read-emails?uuid=${uuid}`);
  };
        ...
        {uuid ? (
          ...
          >
            ...
            <p className="m-0 max-w-[30ch] text-balance text-sm opacity-50">
              Open the emails list to see the emails you have access to.
            </p>
          </button>
        ) : (
          <AuthButton authUrl={authUrl} handleAuthSuccess={handleAuthSuccess} />
        )}
      </div>
    </main>
  );
